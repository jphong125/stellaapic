...
Casino Operator will use provided icons for StartGame method during opening specific game and Bet requests will be also sent using this symbols.
...
| Refund                | Refunds player’s balance. This method will be used for cancellation of a bet in the case the game unfinished round. (i.e. Players connection was lost in any cases so the round is remaining unfinished (pending)) | Required |
| EndRound              | If the Casino Operator requires to finalize the round that is completed in their system with signal received from the Game Provider, then please refer this parameter.                                             | Optional |
