# Round (Game Result =>Betting & Win)

Reporting results of Spin or Free Spins.
...
| transaction.bet       |string|Bet amount (decimal string)|
| transaction.jackpot   |string|Jackpot Win (decimal string)|
| transaction.win       |string|Spin Win (decimal string)|
| transaction.total_win |string|Total Win(win+jackpot) (decimal string)|
| uuid                  |string|A unique ID for each request|

### Example of URL

``` http
https://<Partner website(Domain)/<Round, specified by partner>?authToken=<TOKEN>
```
